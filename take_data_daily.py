...
def EMA_decision(df):
    ...
    (df.Low<=df.EMA50)&(df.Close>=df.EMA50)), 'EMA50_cross'] = 'Buy'
    df.loc[((df.Close<=df.EMA50)& (df.Close.shift(1)>=df.EMA50.shift(1)))|((df.Close.shift(1)<=df.EMA50.shift(1))& \
    (df.High>=df.EMA50)&(df.Close<=df.EMA50)), 'EMA50_cross'] = 'Sell'
    ...
    (df.Low<=df.EMA200)&(df.Close>=df.EMA200)), 'EMA200_cross'] = 'Buy'
    df.loc[((df.Close<=df.EMA200)& (df.Close.shift(1)>=df.EMA200.shift(1)))|((df.Close.shift(1)<=df.EMA200.shift(1))& \
    (df.High>=df.EMA200)&(df.Close<=df.EMA200)), 'EMA200_cross'] = 'Sell'
...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        if  len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value and frame['RISK'].iloc[-1]<=riskvalue :
            ...
            if option1 == 'Buy' and (framew['Close'].iloc[-1]>framew['sup2'].iloc[-1] or framew['Close'].iloc[-1]>framew['sup4'].iloc[-1]) and framew['Dec_EMA20'].iloc[-1]=='Buy'\
            and ((framew['Stochrsi_k'].iloc[-1]>framew['Stochrsi_d'].iloc[-1] and framew['Stochrsi_k'].iloc[-1]>0.2) or framew['MACD_diff'].iloc[-1]>0 or framew['Trend MACD'].iloc[-1]=='Buy'):
                if option2 == 'EMASUPER':  
                    ...
                    and (frame['MACD_diff'].iloc[-1]>0 or frame['Dec_EMA20'].iloc[-1]=='Buy' ): #or frame['Stochrsi_k'].iloc[-1]>frame['Stochrsi_d'].iloc[-1]):   
                    #frame['EMA50_cross'].iloc[-1]=='Buy' or frame['EMA20_cross'].iloc[-1]=='Buy' or frame['EMA200_cross'].iloc[-1]=='Buy'
                    #(frame['Close'].iloc[-1]>frame['EMA20'].iloc[-1] or frame['Close'].iloc[-1]>frame['EMA50'].iloc[-1] or frame['Close'].iloc[-1]>frame['EMA200'].iloc[-1])\
                    #(framew['Dec_EMA50'].iloc[-1]=='Buy' or framew['Dec_EMA20'].iloc[-1]=='Buy')
                            sira +=1
                            expander()
                if option2 == 'EMA200':  
                    if frame['EMA200_cross'].iloc[-1]=='Buy' and frame['MACD_diff'].iloc[-1]>0: #and frame['Trend MACD'].iloc[-1]=='Buy':
                            sira +=1
                            expander()
                if option2 == 'EMA20':
                    if frame['EMA20_cross'].iloc[-1]=='Buy' and frame['MACD_diff'].iloc[-1]>0: # and frame['Trend MACD'].iloc[-1]=='Buy' :
                            sira +=1
                            expander() 
                if option2 == 'ADX':
                ...
                if option2 == 'Supertrend':
                    if (frame['Decision Super'].iloc[-1]=='Buy' or frame['Decision Super2'].iloc[-1]=='Buy' or frame['Decision Super3'].iloc[-1]=='Buy')\
                    and frame['MACD_diff'].iloc[-1]>0 and frame['Dec_EMA50'].iloc[-1]=='Buy': #and frame['Trend MACD'].iloc[-1]=='Buy' :
                            sira +=1
                            expander()
            ...
            if option1 == 'Sell' and framew['Close'].iloc[-1]<framew['sup2'].iloc[-1] and framew['Dec_EMA20'].iloc[-1]=='Sell'\
            ...
            #(framew['Dec_EMA50'].iloc[-1]=='Sell' or framew['Dec_EMA20'].iloc[-1]=='Sell')\    
                if option2 == 'EMASUPER':
                    if (frame['EMA50_cross'].iloc[-1]=='Sell' or frame['EMA20_cross'].iloc[-1]=='Sell' or frame['Decision Super'].iloc[-1]=='Sell'\
                    or frame['Decision Super'].iloc[-1]=='Sell' or frame['Decision Super2'].iloc[-1]=='Sell' or frame['Decision Super3'].iloc[-1]=='Sell')\
                    and (frame['MACD_diff'].iloc[-1]<0 and frame['Close'].iloc[-1]<frame['EMA50'].iloc[-1]):
                            sira +=1
                            expander()
                if option2 == 'EMA200':  
