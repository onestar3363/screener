...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        if  len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value and frame['RISK'].iloc[-1]<=riskvalue :
            ...
            if option1 == 'Buy' and (framew['Dec_EMA50'].iloc[-1]=='Buy' or framew['Dec_EMA20'].iloc[-1]=='Buy') and (framew['MACD_diff'].iloc[-1]>0\
            or (framew['Stochrsi_k'].iloc[-1]>framew['Stochrsi_d'].iloc[-1] and framew['Stochrsi_k'].iloc[-1]>0.2)):
