...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        if  len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value: #and frame['RISK'].iloc[-1]<=riskvalue :
            ...
            and (framew['Stochrsi_k'].iloc[-1]>framew['Stochrsi_d'].iloc[-1] or framew['Trend MACD'].iloc[-1]=='Buy'):
            ...
            and (framew['Stochrsi_k'].iloc[-1]<framew['Stochrsi_d'].iloc[-1] or framew['Trend MACD'].iloc[-1]=='Sell'):   
            #framew['MACD_diff'].iloc[-1]<0
                if option2 == 'EMASUPER':
