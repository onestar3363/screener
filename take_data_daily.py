...
def EMA_decision(df):
    ...
    df.loc[(df.Close<df.EMA50)& (df.Close.shift(1)>df.EMA50.shift(1))|((df.Close.shift(1)<df.EMA50.shift(1))& \
    (df.Low.shift(1)>=df.EMA50.shift(1))&(df.Close<df.EMA50)), 'EMA50_cross'] = 'Sell'
    ...
    (df.Low.shift(1)<=df.EMA200.shift(1))&(df.Close>df.EMA200)), 'EMA200_cross'] = 'Buy'
    df.loc[(df.Close<df.EMA200)& (df.Close.shift(1)>df.EMA200.shift(1))|((df.Close.shift(1)<df.EMA200.shift(1))& \
    (df.Low.shift(1)>=df.EMA200.shift(1))&(df.Close<df.EMA200)), 'EMA200_cross'] = 'Sell'
...
for name, frame,framew in zip(names,framelist,framelistw): 
    if option1 == 'Buy'and option2 == 'EMA':  
        try:
            if len(frame)>30 and len(framew)>30 and (frame['EMA20_cross'].iloc[-1]=='Sell' or frame['EMA50_cross'].iloc[-1]=='Buy' or frame['EMA200_cross'].iloc[-1]=='Buy') \
        ...
        try: 
            if len(frame)>30 and len(framew)>30 and (frame['Dec_MACD'].iloc[-1]=='Sell' or frame['Dec_DIOSQ'].iloc[-1]=='Sell')   \
