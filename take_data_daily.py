...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        if  len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value and frame['RISK'].iloc[-1]<=riskvalue :
            ...
            and (framew['Stochrsi_k'].iloc[-1]>framew['Stochrsi_d'].iloc[-1] or framew['MACD_diff'].iloc[-1]>0 or framew['Trend MACD'].iloc[-1]=='Buy'):
                if option2 == 'EMASUPER':  
                    ...
                    and (frame['Stochrsi_k'].iloc[-1]>frame['Stochrsi_d'].iloc[-1] or frame['MACD_diff'].iloc[-1]>0 or frame['Trend MACD'].iloc[-1]=='Buy' ):
                            sira +=1
                            expander()
            if option1 == 'Sell' and (framew['Dec_EMA20'].iloc[-1]=='Sell' or framew['Dec_EMA50'].iloc[-1]=='Sell')\
            and (framew['Stochrsi_k'].iloc[-1]<framew['Stochrsi_d'].iloc[-1] or framew['Trend MACD'].iloc[-1]=='Sell' or framew['MACD_diff'].iloc[-1]<0 or frame['Trend MACD'].iloc[-1]=='Buy'):   
                if option2 == 'EMASUPER':
                    ...
                    and (frame['Dec_EMA50'].iloc[-1]=='Sell' or frame['Dec_EMA200'].iloc[-1]=='Sell' or frame['Dec_EMA20'].iloc[-1]=='Sell')\
                    and (frame['Stochrsi_k'].iloc[-1]<frame['Stochrsi_d'].iloc[-1] or frame['MACD_diff'].iloc[-1]<0 or frame['Trend MACD'].iloc[-1]=='Sell' ):
                            sira +=1
                            expander()
