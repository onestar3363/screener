...
def Supertrend(df):
    ...
    df.loc[(df.sup==-1)&(df.sup.shift(1)==1)|(df.Close.shift(1)<=df.sup2.shift(1))& \
    (df.High>=df.sup2)&(df.Close<df.sup2)|(df.Low<=df.sup2), 'Decision Super'] = 'Sell' 
    
    df.loc[(df.sup3==1)&(df.sup3.shift(1)==-1)|(df.Close.shift(1)>=df.sup4.shift(1))& \
    (df.Low<=df.sup4)&(df.Close>df.sup4)|(df.High>=df.sup4), 'Decision Super2'] = 'Buy'
    df.loc[(df.sup3==-1)&(df.sup3.shift(1)==1)|(df.Close.shift(1)<=df.sup4.shift(1))& \
    (df.High>=df.sup4)&(df.Close<df.sup4)|(df.Low<=df.sup4), 'Decision Super2'] = 'Sell'   
    
    df.loc[(df.sup5==1)&(df.sup5.shift(1)==-1)|(df.Close.shift(1)>=df.sup6.shift(1))& \
    (df.Low<=df.sup6)&(df.Close>df.sup6)|(df.High>=df.sup6), 'Decision Super3'] = 'Buy'
    df.loc[(df.sup5==-1)&(df.sup5.shift(1)==1)|(df.Close.shift(1)<=df.sup6.shift(1))& \
    (df.High>=df.sup6)&(df.Close<df.sup6)|(df.Low<=df.sup6), 'Decision Super3'] = 'Sell' 
        ...
        if  len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value and frame['RISK'].iloc[-1]<=riskvalue :
            ...
            or framew['MACD_diff'].iloc[-1]>0): 
                if option2 == 'EMASUPER':  
                    ...
                    and (frame['MACD_diff'].iloc[-1]>0 and frame['EMA20'].iloc[-1]>frame['EMA50'].iloc[-1]):                        
                            sira +=1
                            expander()
                if option2 == 'EMA200':  
            ...
            or framew['MACD_diff'].iloc[-1]<0):
                if option2 == 'EMASUPER':  
                    ...
                    or frame['Decision Super2'].iloc[-1]=='Sell' or frame['Decision Super2'].iloc[-1]=='Sell' or frame['Decision Super3'].iloc[-1]=='Sell')\
                    and (frame['MACD_diff'].iloc[-1]<0 and frame['EMA20'].iloc[-1]<frame['EMA50'].iloc[-1]):                            
                            sira +=1
                            expander()
                if option2 == 'EMA200':  
