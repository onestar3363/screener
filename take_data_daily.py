...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        if  len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value and frame['RISK'].iloc[-1]<=riskvalue :
            ...
            and ((framew['Stochrsi_k'].iloc[-1]>framew['Stochrsi_d'].iloc[-1] and framew['Stochrsi_k'].iloc[-1]>0.2) or framew['MACD_diff'].iloc[-1]>0):
                if option2 == 'EMASUPER':  
                    ...
                    and (frame['MACD_diff'].iloc[-1]>0 or frame['Stochrsi_k'].iloc[-1]>frame['Stochrsi_d'].iloc[-1] or frame['Close'].iloc[-1]>frame['sup6'].iloc[-1]\
                    or frame['Close'].iloc[-1]>frame['sup4'].iloc[-1]):   
                            sira +=1
                            expander()
                if option2 == 'EMA200':  
