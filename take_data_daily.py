...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        if  len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value and frame['RISK'].iloc[-1]<=riskvalue :
            ...
            or framew['Stochrsi_k'].iloc[-1]>framew['Stochrsi_d'].iloc[-1]):
            ...
            if option1 == 'Sell' and (framew['Dec_EMA50'].iloc[-1]=='Sell') or (framew['Stochrsi_k'].iloc[-1]<framew['Stochrsi_d'].iloc[-1]\
