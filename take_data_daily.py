...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        if  len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value and frame['RISK'].iloc[-1]<=riskvalue :
            ...
            or framew['MACD_diff'].iloc[-1]>0 or framew['Stochrsi_k'].iloc[-1]>framew['Stochrsi_k'].iloc[-2] or framew['Stochrsi_k'].iloc[-1]>framew['Stochrsi_d'].iloc[-1]):
                ...
                if option2 == 'EMASUPER':  
                    ...
                    and (frame['MACD_diff'].iloc[-1]>0 or frame['Stochrsi_k'].iloc[-1]>frame['Stochrsi_k'].iloc[-2])\
