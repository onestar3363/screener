...
def get_figures(frame,r):
    ...
    fig.add_trace(go.Candlestick(x=frame['Date'].tail(r), open=frame['Open'].tail(r-h1), high=frame['High'].tail(r-h1), low=frame['Low'].tail(r-h1), close=frame['Close'].tail(r-h1)))
...
def expander(cond):
       with st.expander(cond+' '+str(sira) +') '+ name+'/'+' RISK= '+str(frame['RISK'].iloc[-1].round(2))+'/ %ATR='+str(frame['ATR%'].iloc[-1].round(2))+'/ %wATR='+str(framew['ATR%'].iloc[-1].round(2))):
           ...
           col3.write(frame[['Close','ADX','EMA20_cross','EMA50_cross','Decision Super','Decision Super2','Decision Super3','Dec_MACD','Trend MACD','MACD_diff']].tail(r-h1))
...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        ...
        #and frame['RISK'].iloc[-1]<=riskvalue:
            ...
            or framew['Close'].iloc[-1]<framew['sup4'].iloc[-1]):       
            #and (framew['Consolidating2'].iloc[-h1]=='Yes' and framew['Consolidating3'].iloc[-h1]=='Yes'):
