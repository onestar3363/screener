...
def EMA_decision(df):
    ...
    #df.loc[((df.Close.shift(1)>=df.EMA20.shift(1))&(df.Low<=df.EMA20)&(df.Close>=df.EMA20)), 'EMA20_cross'] = 'SBL'
    ...
    df.loc[((df.Close.shift(1)<=df.EMA50.shift(1))&(df.High>=df.EMA50)&(df.Close<=df.EMA50)), 'EMA50_cross'] = 'SBH'
    ...
    df.loc[((df.Close.shift(1)>=df.EMA50.shift(1))&(df.Low<=df.EMA50)&(df.Close>=df.EMA50)), 'EMA50_cross'] = 'SBL'
...
def Supertrend(df):
    ...
    df.loc[(df.Close.shift(1)<=df.sup2.shift(1))&(df.High>=df.sup2)&(df.Close<df.sup2), 'Decision Super'] = 'SBH'
    ...
    df.loc[(df.Close.shift(1)>=df.sup2.shift(1))&(df.Low<=df.sup2)&(df.Close>df.sup2), 'Decision Super'] = 'SBL'
    ...
    df.loc[(df.Close.shift(1)<=df.sup4.shift(1))&(df.High>=df.sup4)&(df.Close<df.sup4), 'Decision Super2'] = 'SBH'
    ...
    df.loc[(df.Close.shift(1)>=df.sup4.shift(1))&(df.Low<=df.sup4)&(df.Close>df.sup4), 'Decision Super2'] = 'SBL'
    ...
    df.loc[(df.Close.shift(1)<=df.sup6.shift(1))&(df.High>=df.sup6)&(df.Close<df.sup6), 'Decision Super3'] = 'SBH'
    ...
    df.loc[(df.Close.shift(1)>=df.sup6.shift(1))&(df.Low<=df.sup6)&(df.Close>df.sup6), 'Decision Super3'] = 'SBL'
...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        if  len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value and frame['RISK'].iloc[-1]<=riskvalue:
            ...
            or framew['Close'].iloc[-1]>framew['sup2'].iloc[-1]) and framew['Dec_EMA20'].iloc[-1]=='Buy':
            ...
            #and (framew['Close'].iloc[-1]>framew['sup4'].iloc[-1] or framew['Close'].iloc[-1]>framew['sup6'].iloc[-1]):
                if option2 == 'EMASUPER':
                    ...
                    or frame['EMA50_cross'].iloc[-1]=='Buy3')\
            ...
            or framew['Close'].iloc[-1]<framew['sup2'].iloc[-1]) and framew['Dec_EMA20'].iloc[-1]=='Sell':
            #or framew['Close'].iloc[-1]<framew['sup2'].iloc[-1]):
            ...
            #and (framew['Close'].iloc[-1]<framew['sup4'].iloc[-1] or framew['Close'].iloc[-1]<framew['sup6'].iloc[-1])
                if option2 == 'EMASUPER':
                   ...
                   or frame['Decision Super2'].iloc[-1]=='Sell' or frame['Decision Super2'].iloc[-1]=='Sell3'\
                   or frame['Decision Super3'].iloc[-1]=='Sell' or frame['Decision Super3'].iloc[-1]=='Sell3')\
