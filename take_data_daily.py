...
def EMA_decision(df):
    ...
    df.loc[((df.Close.shift(1)<=df.EMA20.shift(1))&(df.High>=df.EMA20)), 'EMA20_cross'] = 'Buy3'
...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        if  len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value and frame['RISK'].iloc[-1]<=riskvalue:
            ...
            if option1 == 'Buy' and (framew['Close'].iloc[-1]>framew['sup6'].iloc[-1] and framew['Close'].iloc[-1]>framew['sup4'].iloc[-1] and framew['Close'].iloc[-1]>framew['sup2'].iloc[-1]):
             ...
             #and (framew['Close'].iloc[-1]>framew['sup4'].iloc[-1] or framew['Close'].iloc[-1]>framew['sup6'].iloc[-1]):
                if option2 == 'EMASUPER':  
                    ...
                    #and (frame['Close'].iloc[-1]>frame['sup4'].iloc[-1] or frame['Close'].iloc[-1]>frame['sup6'].iloc[-1]):
                            sira +=1
                            expander()
            if option1 == 'Sell' and (framew['Close'].iloc[-1]<framew['sup6'].iloc[-1] and framew['Close'].iloc[-1]<framew['sup4'].iloc[-1]): #and framew['Close'].iloc[-1]<framew['sup2'].iloc[-1]):
