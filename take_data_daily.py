...
def get_figures(frame):
    ...
    fig.add_trace(go.Candlestick(x=frame['Date'].tail(10), open=frame['Open'], high=frame['High'], low=frame['Low'], close=frame['Close']))
...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        if len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value and frame['RISK'].iloc[-1]<=riskvalue :
            if option1 == 'Buy' and framew['Trend MACD'].iloc[-1]=='Buy' and (framew['Dec_EMA50'].iloc[-1]=='Buy' or framew['sup'].iloc[-1]==1) :
            ...
            #and framew['sup'].iloc[-1]==-1 and framew['Dec_EMA50'].iloc[-1]=='Sell':
                ...
                if option2 == 'Consolidating':
                    if (frame['Consolidating2'].iloc[-1]=='Yes' or frame['Consolidating3'].iloc[-1]=='Yes') \
                    and frame['MACD_diff'].iloc[-1]<0 and framew['Trend MACD'].iloc[-1]=='Sell' and frame['Dec_EMA20'].iloc[-1]=='Sell':
                            sira +=1
                            expander()          
                if option2 == 'Supertrend':
