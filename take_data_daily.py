...
def EMA_decision(df):
    ...
    df.loc[(df.Close.shift(1)>=df.EMA20.shift(1))&(df.Low<=df.EMA20)&(df.Close>=df.EMA20), 'EMA20_cross'] = 'Buy'
    df.loc[(df.Close.shift(1)<=df.EMA20.shift(1))&(df.High>=df.EMA20)&(df.Close<=df.EMA20), 'EMA20_cross'] = 'Sell' 
    ...
    df.loc[(df.Close.shift(1)>=df.EMA50.shift(1))&(df.Low<=df.EMA50)&(df.Close>=df.EMA50), 'EMA50_cross'] = 'Buy'
    df.loc[(df.Close.shift(1)<=df.EMA50.shift(1))&(df.High>=df.EMA50)&(df.Close<=df.EMA50), 'EMA50_cross'] = 'Sell' 
...
for name, frame,framew in zip(names,framelist,framelistw): 
    try:
        if  len(frame)>30 and len(framew)>30 and frame['ADX'].iloc[-1]>=adx_value and frame['RISK'].iloc[-1]<=riskvalue:
            ...
            and (framew['Dec_EMA20'].iloc[-1]=='Buy' or framew['Dec_EMA50'].iloc[-1]=='Buy' or framew['Close'].iloc[-1]>framew['sup2'].iloc[-1]):
            #or framew['Close'].iloc[-1]>framew['sup2'].iloc[-1]):
                if option2 == 'EMASUPER':
                    ...
                    or frame['EMA50_cross'].iloc[-1]=='SB' or frame['EMA20_cross'].iloc[-1]=='SB')\
                    ...
                    and (frame['Dec_EMA20'].iloc[-1]=='Buy' or frame['Dec_EMA50'].iloc[-1]=='Buy'):
                            sira +=1
                            expander()
                    elif (frame['Consolidating2'].iloc[-1]=='Yes' or frame['Consolidating3'].iloc[-1]=='Yes')\
            ...
            #(framew['Dec_EMA20'].iloc[-1]=='Sell' or framew['Dec_EMA50'].iloc[-1]=='Sell'\
            #or framew['Close'].iloc[-1]<framew['sup2'].iloc[-1] or
